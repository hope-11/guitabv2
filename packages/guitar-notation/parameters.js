...
SVG.guitarTab = {
    ...
    //和弦名称文字样式
    chordTitleTextFont : {size: 16, anchor: 'middle'},
    ...
    freds: function () {
        let arr = [];
        for (let i = 1, n = this.fredNum; i <= n; i++) {
            arr.push(i);
        }
    ...
    chordHeight: function (chord) {
        let chordFredNumber = 3;
        if (typeof chord !== 'undefined') {
            chordFredNumber = this.chordFredNumber(chord)
        }

        return this.chordTitleTextFont.size + this.chordFredDistance * chordFredNumber;
    },
     ...
     */
    chordTitleTextPosition: function () {
        let x = this.chordGridWidth() / 2;
        let y = - this.chordTitleTextFont.size;
        return {x: x, y: y};
    },
     ...
     * @returns {*}
     */
    tabHeadHeight: function(guitarNum) {
        return (this.guitarTabHeight() + this.tabDistance()) * guitarNum + this.numberedTabHeight();
    },
     ...
     * @returns {number}
     */
    numberedLineDistance: function () {
        return this.numberedDotRadius * 2;
    },
     ...
     * @returns {number}
     */
    numberedLineLength: function () {
        return this.numberedTextFont.size / 2;
    }

    ...
    extend: {
        numberedBar: function (x, y, width, barData, beatPerBar, notePerBeat) {
            ...
            for (let i = 0; i < noteNum; i++) {
                ...
                //绘制音符
                let noteSymbol = this.numberedNote(note, timer).move(noteX, noteY);

                //音符底部横线的位置、距离、长度
                let noteLineY = noteSymbol.lineY;
                let noteLineDistance = guitarTab.numberedLineDistance();
                let noteLineLength = guitarTab.numberedLineLength();

...
SVG.GuitarChord = SVG.invent({
    ...
    extend: {
         ...
         * @returns {*}
         */
        drawGuitarChord: function (chord) {
            ...
            let titleTextFont = guitarTab.chordTitleTextFont;
            ...
            let titleTextPosition = guitarTab.chordTitleTextPosition();

            //绘制和弦名称
            this.text(chord.title)
                .font(titleTextFont)
                .move(titleTextPosition.x, titleTextPosition.y);
            ...
            for (let i = 0, n = chord.fingers.length; i < n; i++) {
                ...
                if (!$.isEmptyObject(finger)) {
                    //指位坐标
                    let fingerCirclePosition = guitarTab.chordFingerCirclePosition(finger.fred, finger.wires[0]);
                    ...
                    if (finger.wires[1]) {
                        ...
                        //横按结束弦位，一般为1弦
                        let xFingerEnd = guitarTab.chordFingerCirclePosition(finger.fred, finger.wires[1]).x + fingerCircleRadius;
    ...
    construct: {
        guitarChord: function (chord) {
            return this.put(new SVG.GuitarChord).drawGuitarChord(chord);
        }
    }
});

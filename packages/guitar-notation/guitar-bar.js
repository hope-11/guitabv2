...
SVG.GuitarBar = SVG.invent({
    ...
    extend: {
        ...
        guitarBarWires: function (x, y, width) {
            ...
            for (let i = 0, n = wireNum; i < n; i++) {
                this.rect(width, wireWidth).move(x, y + i * wireDistance);
            }
            this.rect(wireWidth, wireDistance * (wireNum - 1) + wireWidth).move(x + width - 1, y);
        ...
        drawGuitarBar: function(x, y, width, barData, beatPerBar, notePerBeat) {
            ...
            if (typeof barData !== 'undefined' && barData.length > 0) {
                ...
                for (let i = 0; i < noteNum; i++) {
                    ...
                    //绘制击弦图形
                    let guitarNote = this.guitarNote(picks, currentNoteTimer).move(pickX, y);
                    /*
                    if (typeof legato === 'boolean') {

                        if (legato) {
                            guitarTab.legatos[guitarTab.legatos.length] = guitarNote.position;
                        } else {
                            console.log(guitarNote.position)
                            guitarTab.legatos = guitarTab.legatos[guitarTab.legatos.length - 1].concat(guitarNote.position);
                            console.log(guitarTab.legatos)
                        }
                    }
                    */

                    //连接线绘图在画板上的纵坐标
                    let hookY = y + guitarTab.guitarWireDistance * (6 + 1/2);
                    ...
                    if (currentNoteTimer < 32) {
                        ...
                        if (lastSumTimer === 0){
                            ...
                            this.guitarNoteHook(pickX, hookY, noteDistance / 2, currentNoteTimer);
                            ...
                            if (currentNoteTimer === lastNoteTimer && currentNoteTimer < nextNoteTimer) {
                                ...
                                this.guitarNoteHook(pickX, hookY, - noteDistance / 2, currentNoteTimer);
                                //右边按照 后面音符 绘线
                                this.guitarNoteHook(pickX, hookY, noteDistance / 2, nextNoteTimer);
                                ...
                                //左边按照 前面音符 绘线
                                this.guitarNoteHook(pickX, hookY, - noteDistance / 2, lastNoteTimer);
                                //右边按照 当前音符 绘线
                                this.guitarNoteHook(pickX, hookY, noteDistance / 2, currentNoteTimer);
                    ...
                    if (typeof chord === 'object') {
                        ...
                        this.guitarChord(chord).move(pickX - guitarWireDistance * 2/3, y - guitarWireDistance - chordGridHeight);
                    }
                }
            }
        }
    ...
    construct: {
        guitarBar: function (x, y, width, barData, beatPerBar, notePerBeat) {
            return this.put(new SVG.GuitarBar).drawGuitarBar(x, y, width, barData, beatPerBar, notePerBeat);
        }
    }
});
